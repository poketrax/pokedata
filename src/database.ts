...
export function getLatestExpansions(num: number): Expansion[] {
  return db
    .prepare(`SELECT * FROM expansions ORDER BY datetime(releaseDate) DESC LIMIT ${num}`)
    .all();
}
...
export function findTcgpCard(tcgpId: number): Card | null {
  let card = db
    .prepare(`SELECT * FROM cards WHERE idTCGP = $idTCGP`)
    .get({ idTCGP: tcgpId.toFixed(0) });
...
export function findCardComplex(setName: string, setNumber: string): Card | undefined {
  let card = db
    .prepare(
      `SELECT * FROM cards WHERE expName = $setName AND (expCardNumber = $setNumber OR expCardNumber = $deNormSetNumber)`
    )
    .get({
      setName: setName,
      setNumber: setNumber,
      deNormSetNumber: setNumber.replaceAll("0", ""),
    });
  if (card == null) return null;
  let _card = {
    cardId: card.cardId,
...
export function expantionExistsInDB(name: string): string | undefined {
  ...
  if (found === false) {
    ...
    let matches = [];
    for (let exp of exps) {
      let confidence: number = stringSimilarity.compareTwoStrings(
        normalizeSetName(exp.name),
        normalizeSetName(name)
      );
      if (confidence > 0.6) {
        logger.info(
          `Expantion ${name} already found with ${(confidence * 100).toFixed(0)}% confidence: ${
            exp.name
          }`
        );
...
export function getLatestPrice(cardId: string) {
  return pricedb
    .prepare("SELECT * FROM prices WHERE cardId = $cardId ORDER BY date(date)")
    .get({ cardId: cardId });
}
 ...
 * @returns
 */
export function getPricesComplex(
  relStart: Date,
  relEnd: Date,
  priceFilter: Date,
  rare?: boolean
...
export function getSealedProducts(name: string, limit: number): SealedProduct[] {
  let name_q = `%${name}%`;
  let limit_q = limit;
  if (name == null) name_q = "%%";
  if (limit == null) limit_q = 100;
  let query = `SELECT * FROM "sealed" WHERE name like $name ORDER BY date(releaseDate) DESC limit $limit`;
