...
import { formatExpNumber, setUpLogger, logger } from "../src/common.js";
import { SealedProduct } from "../src/model/SealedProduct.js";
import { assert } from "console";
import {
  getRecentProducts as getWalmartProducts,
  SEARCH_BASE as WALMART_BASE,
} from "../src/scrappers/walmart-scrapper.js";
...
describe("Scrape Serebii data", () => {
  ...
  it("scrape serebii promo sets", async () => {
    return getSerebiiLastestPromoExpantions(5).then((val) => console.log(JSON.stringify(val, null, 1)));
  }).timeout(10000);
  it("scrape serebii cards", async () => {
    ...
    return getSerebiiSetCards(testSet, exp)
      .then((data) => {
        console.log(data[0]);
        expect(data.length).to.be.equal(exp.numberOfCards);
      })
      .catch((e) => expect.fail(`Error scraping serebii ${e.stack}`));
  });
...
describe("Scrape ebay prices", () => {
  it("should scrape raw ebay price", async () => {
    let card = testPromoCard();
    let data = await scrapeEbay(card, "raw");
  ...
  it("should scrape grade 9 data", async () => {
    let card = testPromoCard();
    let data = await scrapeEbay(card, "grade9");
    logger.info(`raw data: ${data}`);
    expect(data).to.be.not.null;
    expect(data).to.be.a("number");
  });
  it("should scrape grade 10 data", async () => {
    let card = testPromoCard();
    let data = await scrapeEbay(card, "grade10");
    console.log("blam")
    logger.info(`raw data: ${data}`);
    expect(data).to.be.not.null;
    expect(data).to.be.a("number");
  });
  it("test multiple cards", async () => {
    console.log()
    let cards = testCards();
    for (let card of cards ?? []) {
      let _card = card as Card;
      let data = await scrapeEbay(_card, "raw");
      logger.info(`raw data: ${data}`);
      expect(data).to.be.not.null;
      expect(data).to.be.a("number");
    }
...
function testCards(): [] | undefined {
  return JSON.parse(
    `[{
      "name": "Quaquaval ex - 260/193",
      "cardId": "Paldea-Evolved-Quaquaval-ex---260",
      "idTCGP": 497679,
      "expName": "Paldea Evolved",
      "expCardNumber": "260",
      "expCodeTCGP": "",
      ...
      "expIdTCGP": "sv02-paldea-evolved",
      "pokedex": 0,
      "releaseDate": "2023-06-09T00:00:00Z",
      "price": 0.0,
      "img": "https://www.serebii.net/card/paldeaevolved/260.jpg",
      "tags": null,
      "variant": null,
      "paid": null,
      "count": null,
    ...
    {
      "name": "Tyranitar - 222/193",
      "cardId": "Paldea-Evolved-Tyranitar---222",
      "idTCGP": 497625,
      "expName": "Paldea Evolved",
      "expCardNumber": "222",
      "expCodeTCGP": "",
      ...
      "releaseDate": "2023-06-09T00:00:00Z",
      "price": 0.0,
      "img": "https://www.serebii.net/card/paldeaevolved/222.jpg",
      "tags": null,
      "variant": null,
      "paid": null,
      "count": null,
